...
class SqlEditor(Gtk.Box):
    ...
    def _build_ui(self):
        ...
        _results_card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        _results_card.add_css_class('card')
        _results_card.set_vexpand(True)
        _results_card.set_margin_start(6)
        _results_card.set_margin_end(6)
        _results_card.set_margin_top(6)
        _results_card.set_margin_bottom(6)
        _results_card.append(self._results_scroll)
        self._results_stack.add_named(_results_card, 'grid')
    ...
    def _show_multi_results(self, results):
        ...
        for i, result in enumerate(results):
            ...
            if result['kind'] == 'select':
                ...
                tab_card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
                tab_card.add_css_class('card')
                tab_card.set_vexpand(True)
                tab_card.set_margin_start(6)
                tab_card.set_margin_end(6)
                tab_card.set_margin_top(6)
                tab_card.set_margin_bottom(6)
                tab_card.append(tab_scroll)
                tab_page = self._results_tab_view.append(tab_card)
